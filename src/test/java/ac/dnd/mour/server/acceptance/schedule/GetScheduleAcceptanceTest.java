...
import static ac.dnd.mour.server.acceptance.schedule.ScheduleAcceptanceStep.알람_동기화를_위한_일정을_조회한다_V1;
import static ac.dnd.mour.server.acceptance.schedule.ScheduleAcceptanceStep.일정_상세_정보를_조회한다_V1;
...
import static ac.dnd.mour.server.acceptance.schedule.ScheduleAcceptanceStep.일정을_숨긴다_V1;
import static ac.dnd.mour.server.acceptance.schedule.ScheduleAcceptanceStep.지출이_기록되지_않은_일정을_조회한다_V1;
import static ac.dnd.mour.server.acceptance.schedule.ScheduleAcceptanceStep.캘린더_Year_Month에_해당하는_일정을_조회한다_V1;
...
public class GetScheduleAcceptanceTest extends AcceptanceTest {
    ...
    class GetScheduleDetails {
        ...
        void success() {
            ...
            final long scheduleId = ScheduleAcceptanceStep.일정을_생성하고_ID를_추출한다_V1(relationId, 결혼식, member.accessToken());

            일정_상세_정보를_조회한다_V1(scheduleId, member.accessToken())
                    .statusCode(OK.value())
    ...
    class GetUnrecordedSchedule {
        ...
        void success() {
            final AuthMember member = MEMBER_1.회원가입과_로그인을_진행한다();
            final long groupId = 관리하고_있는_특정_그룹의_ID를_조회한다_V1("친구", member.accessToken());
            final long relationId1 = 관계를_생성하고_ID를_추출한다_V1(groupId, "관계-친구XXX-1", null, null, member.accessToken());
            final long relationId2 = 관계를_생성하고_ID를_추출한다_V1(groupId, "관계-친구XXX-2", null, null, member.accessToken());
            final long relationId3 = 관계를_생성하고_ID를_추출한다_V1(groupId, "관계-친구XXX-3", null, null, member.accessToken());
            final long relationId4 = 관계를_생성하고_ID를_추출한다_V1(groupId, "관계-친구XXX-4", null, null, member.accessToken());

            final long scheduleId1 = 일정을_생성하고_ID를_추출한다_V1(relationId1, LocalDate.of(now.getYear() + 1, 1, 1), "일정1", 특별한_일정_XXX, member.accessToken());
            final long scheduleId2 = 일정을_생성하고_ID를_추출한다_V1(relationId2, LocalDate.of(now.getYear() + 1, 1, 15), "일정2", 특별한_일정_XXX, member.accessToken());
            final long scheduleId3 = 일정을_생성하고_ID를_추출한다_V1(relationId3, LocalDate.of(now.getYear() + 1, 1, 18), "일정3", 특별한_일정_XXX, member.accessToken());
            final long scheduleId4 = 일정을_생성하고_ID를_추출한다_V1(relationId4, LocalDate.of(now.getYear() + 1, 1, 22), "일정4", 특별한_일정_XXX, member.accessToken());

            final ValidatableResponse response1 = 지출이_기록되지_않은_일정을_조회한다_V1(member.accessToken()).statusCode(OK.value());
            ...
            지출이_기록되지_않는_일정에_대한_마음을_생성한다_V1(scheduleId2, 100_000_000, List.of("특별한 일정", "이제 기록", "2"), member.accessToken());
            final ValidatableResponse response2 = 지출이_기록되지_않은_일정을_조회한다_V1(member.accessToken()).statusCode(OK.value());
            assertUnrecordedSchedulesMatch(
                    response2,
            ...
            일정을_숨긴다_V1(scheduleId1, member.accessToken());
            final ValidatableResponse response3 = 지출이_기록되지_않은_일정을_조회한다_V1(member.accessToken()).statusCode(OK.value());
            assertUnrecordedSchedulesMatch(
                    response3,
                    List.of(scheduleId3),
                    List.of(relationId3),
            ...
            지출이_기록되지_않는_일정에_대한_마음을_생성한다_V1(scheduleId3, 300_000_000, List.of("특별한 일정", "이제 기록", "1"), member.accessToken());
            final ValidatableResponse response4 = 지출이_기록되지_않은_일정을_조회한다_V1(member.accessToken()).statusCode(OK.value());
    ...
    class GetCalendarSchedule {
        ...
        void success() {
            ...
            final long scheduleId1 = ScheduleAcceptanceStep.일정을_생성하고_ID를_추출한다_V1(relationId1, 결혼식, LocalDate.of(now.getYear() + 1, 1, 1), member.accessToken());
            final long scheduleId2 = ScheduleAcceptanceStep.일정을_생성하고_ID를_추출한다_V1(relationId2, 특별한_일정_XXX, LocalDate.of(now.getYear() + 1, 1, 15), member.accessToken());
            final long scheduleId3 = ScheduleAcceptanceStep.일정을_생성하고_ID를_추출한다_V1(relationId3, 친구_XXX_생일, LocalDate.of(now.getYear() + 1, 2, 1), member.accessToken());

            final ValidatableResponse response1 = 캘린더_Year_Month에_해당하는_일정을_조회한다_V1(now.getYear() + 1, 1, member.accessToken());
            ...
            final ValidatableResponse response2 = 캘린더_Year_Month에_해당하는_일정을_조회한다_V1(now.getYear() + 1, 2, member.accessToken());
            assertCalendarSchedulesMatch(
                    response2,
                    List.of(scheduleId3),
                    List.of(relationId3),
            ...
            final ValidatableResponse response3 = 캘린더_Year_Month에_해당하는_일정을_조회한다_V1(now.getYear() + 1, 3, member.accessToken());
    ...
    class GetSchedulesForAlarm {
        ...
        void success() {
            ...
            final long scheduleId1 = ScheduleAcceptanceStep.일정을_생성하고_ID를_추출한다_V1(relationId1, 결혼식, member.accessToken());
            final long scheduleId2 = ScheduleAcceptanceStep.일정을_생성하고_ID를_추출한다_V1(relationId2, 특별한_일정_XXX, member.accessToken()); // alarm null
            final long scheduleId3 = ScheduleAcceptanceStep.일정을_생성하고_ID를_추출한다_V1(relationId3, 친구_XXX_생일, member.accessToken());

            final ValidatableResponse response1 = 알람_동기화를_위한_일정을_조회한다_V1(member.accessToken());
            ...
            일정을_삭제한다_V1(scheduleId3, member.accessToken());
            final ValidatableResponse response2 = 알람_동기화를_위한_일정을_조회한다_V1(member.accessToken());
            assertSchedulesForAlarmMatch(
                    response2,
                    List.of(scheduleId1),
                    List.of(relationId1),
